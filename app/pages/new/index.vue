...
import {ref} from "vue";
import {getByArtistID, submit} from "~/services/art";
//#endregion
...
const checkArtsCount = async () => {
  const response = await getByArtistID(user?.id,{page:1,pageSize:9999,search:''})
  if (response.result) {
    if (response.data.results.length > 0) {
      await router.push('/error?code=1')
    }
    else return false;
  }else{
    return false;
...
<template>
      ...
      <div  :class="isDesktop ? '!px-30':''" class="fixed top-0 left-0 bg-gray-900 z-50 flex w-full justify-between items-center p-2">
        ...
        <div class="flex justify-center items-center">
          <div class=" rounded-full bg-blue-800 p-3" @click="handleSubmit">
            <Check class="text-xs size-5 text-gray-200"/>
          </div>
        </div>
        <!--#endregion-->
  ...
  <div v-else class="flex justify-center items-center p-5">
    <span class="text-gray-200 Farhang_Bold"> در حال بررسی ... </span>
  </div>
</template>
